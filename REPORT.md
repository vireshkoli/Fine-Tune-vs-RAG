...
(≤ 8 of 300 at 96 tokens), which matters because the MIRIAD arms in §2.5 do.
...
### 2.5 Literal information parity on MIRIAD — in progress

§2.1's parity is *topical*: the index holds passages about the test questions'
subjects, and the fine-tune saw MedMCQA training items. The strict version of
the question — the same passages in the weights and in the index — is built
from MIRIAD itself: 17,006 passages, one QA pair per passage held out as the
test set (300 items), the remaining 30,001 QA pairs as fine-tuning data, and
every one of those passages chunked into a 189,031-chunk index. Two adapters
ask the question twice: `miriad-qa` trained on the question→answer pairs,
`miriad-doc` trained on the raw passages (causal LM on chunks). Six arms
(base, index, each adapter with and without the index), free-text, judged as
in §2.4 with the generation bound raised to 256 tokens because MIRIAD
references run to 212. `miriad-doc` is training (11,802 steps); this section
is filled in by `scripts/15_freetext_stats.py --dataset miriad` when the six
runs are judged.
