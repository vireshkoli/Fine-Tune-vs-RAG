...
- Two adapters, so "weights" is asked two ways: `miriad-qa`, trained on the
  30,001 question→answer pairs (the format a fine-tune is usually given), and
  `miriad-doc`, trained on the raw passages as a causal LM — one epoch over
  every chunk, the format "just put the documents in the weights" implies.
  Same rank, same recipe as every other adapter here.
...
- Free-text only — MIRIAD has no options — judged exactly as §2.4, with the
  ...
  / max 212 tokens, and at 96 the QA-trained arm was being cut off mid-sentence
...
`scripts/15_freetext_stats.py --dataset miriad`.

**On literally identical information, the index beats the weights by 20 to 29
points, whichever way the weights were trained.** Against the QA-trained
adapter, +28.6 (pairwise 190–10 after excluding order-flipped verdicts);
against the passage-trained adapter, +20.4 (177–20). The two adapters fail
differently and the difference is the finding:

- **Training on the passages did nothing.** One epoch of causal-LM QLoRA over
  the exact 17,006 passages the questions come from leaves free-text quality
  where the base model had it: 0.674 vs 0.680, p = 0.75, pairwise 28–26. The
  weights did not absorb the content of documents they were trained on, at
  rank 16 and one epoch — which is what most "fine-tune on our docs" budgets
  look like.
...
MIRIAD answers are paragraphs. The QA-trained adapter writes in the corpus's
register — long, specific, assured, twice the base model's length — and the
specifics are invented: "CHI3L1 deficiency *reduces* the severity of chronic
colitis" where the passage says it exacerbates it; a metabolite list that
degenerates into "2-hydroxyglutarate, 3-hydroxyglutarate, 4-hydroxyglutarate,
5-…". The base model hedges in general terms and collects partial credit.
Length is not what the judge is rewarding: within each arm, answers scored
high and low are the same length (94 vs 96 tokens for the QA adapter, 49 vs
51 for base). The passage-trained adapter, by contrast, answers at base's
length (37 tokens) in base's register — its training data had no answers in
it to imitate — and scores like base. One epoch of QLoRA taught the *shape*
of the training data far more reliably than any of its content, and when the
shape was "confident specific paragraph" that cost points. This is what a
rank-16 adapter on 8B frozen weights should be expected to learn, and is the
point.
